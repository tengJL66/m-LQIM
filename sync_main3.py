...
if __name__ == '__main__':
    ...
    for num in range(50):
        ...
        all_sync = [sequence[i:i + sync_chunk_size] for i in range(0, len(sequence), sync_chunk_size)][
                   0:seg_num]
        ...
        for i in range(len(W_seg)):
            for j in range(sync_chunk_size):
                wm_signal[i * (p) + j:i * (p) + j + 1] = standard_qim(wm_signal[i * (p) + j:i * (p) + j + 1],
                                                                      b1,
                                                                      all_sync[i][j], alpha1,
        ...
        with Pool(processes=num_processes) as pool:
            #     并行解码
            results = pool.starmap(find_sequence,
                                   [(trans_wmsignal, template) for template in all_sync[0:seg_num]])
        ...
        if M_ext_sequence == -1:
            ...
            # ber_list.append(ber)
            # corr_list.append(calculate_ncorr(np.asarray(W_seg).reshape((50,50)), np.asarray(extracted_watermark).reshape((50,50))))
            continue
        # 将移位的M序列分成两部分
        ...
        ber_list.append(ber)
        corr_list.append(calculate_ncorr(W_ori_flat.reshape((50,50)), restored_watermark.reshape((50,50))))
