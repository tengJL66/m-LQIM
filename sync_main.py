...
if __name__ == '__main__':
    ...
    # # MP3
    # wm_signal_64k = "audio_file/wm_signal_64k.mp3"
    # wm_signal_128k = "audio_file/wm_signal_128k.mp3"
    # compress_mp3("audio_file/wm_signal.wav",wm_signal_64k,"64K")
    # compress_mp3("audio_file/wm_signal.wav", wm_signal_128k, "128K")
    ...
    # 缩放攻击
    # wm_signal[0:wm_signal_len] = np.copy(wm_signal[0:wm_signal_len]) * 0.7
    ...
    # 再次重塑为50x50
    data_reshaped = np.asarray(extract_W)
    ...
    # 计算错误比特数
    error_bits = np.sum(matrix != W_o)
    # 计算总比特数
    total_bits = matrix.size
    # 计算误码率
    ber = error_bits / total_bits
    print("水印BER:", ber)
    print("水印corr:", calculate_ncorr(matrix.flatten(), W))

    ber_list.append(ber)
    corr_list.append(calculate_ncorr(W, matrix.flatten()))
    ...
    for i in range(len(extracted_watermark)):
        if not np.array_equal(extracted_watermark[i], -1) and np.all(extracted_watermark[i][0:L // L_bit] != -9999):
            temp = np.array([])
...
"""
    ...
    print("mse", mse_list)
    print(sum(mse_list) /len(odg_list))

    print("ber", ber_list)
    print(sum(ber_list) / len(odg_list))

    print("ncorr", corr_list)
    print(sum(corr_list) / len(odg_list))
